...
function getHeight(state, index) {
  return (state.rowState ? state.rowState[index] : DEFAULT_HEIGHT) + 'px'
}
  ...
  return function(_, col) {
    ...
    return `
      <div class="cell" contenteditable data-col='${col}'
        data-id='${id}' data-type="cell" data-value="${data || ''}"
...
export function createTable(rowsCount = 15, state) {
  ...
  const cols = new Array(colsCount)
      ...
      .map(toColumn)
      .join('')
  rows.push(createRow(state.rowState, null, cols))
  ...
  for (let row=0; row<rowsCount; row++) {
    const cols = new Array(colsCount)
        ...
        .join('')
    rows.push(createRow(state.rowState, row+1, cols))
  }
